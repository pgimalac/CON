...
static void fillCircles(SDL_Renderer *renderer, int size, int xx[size],
                        int yy[size], int radius) {
    for (int i = 0; i < radius; i++) {
        for (int j = 0; i >= j && (i * i) + (j * j) <= (radius * radius); j++) {
            for (int k = 0; k < size; k++) {
                if (SDL_RenderDrawPoint(renderer, xx[k] + i, yy[k] + j))
                    fprintf(stderr, "%s\n", SDL_GetError());
                if (SDL_RenderDrawPoint(renderer, xx[k] + i, yy[k] - j))
                    fprintf(stderr, "%s\n", SDL_GetError());
                if (SDL_RenderDrawPoint(renderer, xx[k] - i, yy[k] + j))
                ...
                if (SDL_RenderDrawPoint(renderer, xx[k] - i, yy[k] - j))
                    fprintf(stderr, "%s\n", SDL_GetError());
                if (SDL_RenderDrawPoint(renderer, xx[k] + j, yy[k] + i))
                    fprintf(stderr, "%s\n", SDL_GetError());
                if (SDL_RenderDrawPoint(renderer, xx[k] + j, yy[k] - i))
                    fprintf(stderr, "%s\n", SDL_GetError());
                if (SDL_RenderDrawPoint(renderer, xx[k] - j, yy[k] + i))
...
void printMenu(View *view) {
    if (SDL_SetRenderDrawColor(view->renderer, HEADER_PANEL_R, HEADER_PANEL_G,
                               HEADER_PANEL_B, HEADER_PANEL_A))
        fprintf(stderr, "%s\n", SDL_GetError());
    if (SDL_RenderFillRect(view->renderer, view->header_panel))
        fprintf(stderr, "%s\n", SDL_GetError());

    if (SDL_SetRenderDrawColor(view->renderer, QUIT_BUTTON_R, QUIT_BUTTON_G,
                               QUIT_BUTTON_B, QUIT_BUTTON_A))
        fprintf(stderr, "%s\n", SDL_GetError());
    if (SDL_RenderFillRect(view->renderer, view->quitButton))
        fprintf(stderr, "%s\n", SDL_GetError());

    if (SDL_SetRenderDrawColor(view->renderer, START_OVER_BUTTON_R,
                               START_OVER_BUTTON_G, START_OVER_BUTTON_B,
                               START_OVER_BUTTON_A))
        fprintf(stderr, "%s\n", SDL_GetError());
    if (SDL_RenderFillRect(view->renderer, view->startOverButton))
        fprintf(stderr, "%s\n", SDL_GetError());

    if (SDL_SetRenderDrawColor(view->renderer, UNDO_BUTTON_R, UNDO_BUTTON_G,
                               UNDO_BUTTON_B, UNDO_BUTTON_A))
        fprintf(stderr, "%s\n", SDL_GetError());
    if (SDL_RenderFillRect(view->renderer, view->undoButton))
        fprintf(stderr, "%s\n", SDL_GetError());

    if (SDL_SetRenderDrawColor(view->renderer, MENU_BUTTON_R, MENU_BUTTON_G,
                               MENU_BUTTON_B, MENU_BUTTON_A))
        fprintf(stderr, "%s\n", SDL_GetError());
...
View *getView(Uint8 pos_h) {
    if (SDL_Init(SDL_INIT_VIDEO)) {
        fprintf(stderr,
                "Erreur au chargement de la bibliothèque graphique. %s\n",
                SDL_GetError());
        exit(EXIT_FAILURE);
    }
    ...
    SDL_Window *screen =
        SDL_CreateWindow("Othello", x, SDL_WINDOWPOS_CENTERED, BOARD_SIZE,
                         BOARD_SIZE + PANEL_SIZE, 0);
    ...
    view->quitButton =
        getSDLRect(QUIT_BUTTON_X, QUIT_BUTTON_Y, QUIT_BUTTON_W, QUIT_BUTTON_H);
    view->startOverButton =
        getSDLRect(START_OVER_BUTTON_X, START_OVER_BUTTON_Y,
                   START_OVER_BUTTON_W, START_OVER_BUTTON_H);
    view->undoButton =
        getSDLRect(UNDO_BUTTON_X, UNDO_BUTTON_Y, UNDO_BUTTON_W, UNDO_BUTTON_H);
    view->menuButton =
        getSDLRect(MENU_BUTTON_X, MENU_BUTTON_Y, MENU_BUTTON_W, MENU_BUTTON_H);
    view->currentPlayer = getSDLRect(CURRENT_PLAYER_X, CURRENT_PLAYER_Y,
                                     CURRENT_PLAYER_W, CURRENT_PLAYER_H);
...
void printPieces(SDL_Renderer *renderer, Uint8 board[8][8], Uint8 player) {
    ...
    if (n1d) {
        if (SDL_SetRenderDrawColor(renderer, PLAYER1_R, PLAYER1_G, PLAYER1_B,
                                   PLAYER1_A))
            fprintf(stderr, "%s\n", SDL_GetError());
        fillCircles(renderer, n1d, xx1, yy1, RADIUS);
    }
    if (n1e) {
        if (SDL_SetRenderDrawColor(renderer, PLAYER2_R, PLAYER2_G, PLAYER2_B,
                                   PLAYER2_A))
            fprintf(stderr, "%s\n", SDL_GetError());
    ...
    if (n2e) {
        if (SDL_SetRenderDrawColor(renderer, POTENTIAL_R, POTENTIAL_G,
                                   POTENTIAL_B, POTENTIAL_A))
            fprintf(stderr, "%s\n", SDL_GetError());
