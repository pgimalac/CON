...
void handle_mouse(SDL_MouseButtonEvent mouse_event){
    ...
    if ((game->gameType == ONLINE_GAME ? game->current_player == game->role : game->current_player != NONE) &&
        button_clicked(mouse_event.x, mouse_event.y, LINE_SIZE / 2, PANEL_SIZE + LINE_SIZE / 2, BOARD_SIZE - LINE_SIZE, BOARD_SIZE - LINE_SIZE)) {
    ...
    else if (button_clicked(mouse_event.x, mouse_event.y, MENU_BUTTON_X, MENU_BUTTON_Y, MENU_BUTTON_W, MENU_BUTTON_H)) {
